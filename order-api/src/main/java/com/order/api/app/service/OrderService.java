...
public class OrderService {
	...
	public ResponseEntity<List<RestaurantInfo>> searchByName(Map<String, String> entity) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			common.addAll(orderRepository.findByRestaurantnameContaining(words[i],
	...
	public ResponseEntity<List<SearchFoodItem>> searchByFoodItem(Map<String, String> entity) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			common.addAll(
					foodItemRepo.findByFoodnameContaining(words[i], Sort.by(Sort.Direction.DESC, "fooditemrating")));
		}
	...
	public ResponseEntity<String> placeOrder(Map entity) {
		...
		for (String id : fooditemid) {
			Optional<FoodItem> foodItem = foodItemRepo.findById(Integer.parseInt(id));
			if (foodItem.isEmpty()) {
		...
		// Initialize iterators for food items and their details
		...
		List<String> quantityStrings = quantity.stream().map(String::valueOf).collect(Collectors.toList());
		...
		while (ll.hasNext()) {
			...
			orderFoodItems.setFoodname(name.next());
	...
	public ResponseEntity<String> rateOrder(Map entity) {
		Optional<RestaurantInfo> restaurantInfo = restaurantInfoRepo.findById((Integer) entity.get("restaurantid"));
		Optional<UserInfo> userInfo = Optional.ofNullable(finduserInfo(entity));// calling user-api for unserInfo
		...
		if (rest.getRestaurantrating() == 0.0) {
			...
			///////////////////
			rating = (float) (((rest.getRestaurantrating() * rest.getNumofrating()) + f1)
		...
		if (fooditemid.isEmpty()) {
			...
			ArrayList<String> numofrating = (ArrayList<String>) entity.get("numofrating");
			ListIterator<String> nrating;

			// Check if numofrating is null, and if so, assign a default value of "1"
			if (numofrating != null) {
			    nrating = numofrating.listIterator();
			} else {
			    // Create a new ArrayList with a single element "1" if numofrating is null
			    numofrating = new ArrayList<>(Arrays.asList("1"));
			    nrating = numofrating.listIterator();
			}
			...
			while (ll.hasNext()) {
				...
				String rate = ratingFood.next();
				String nRate = nrating.next();
				logger.info("########################" + rate);
				logger.info("***************************" + Double.parseDouble(rate));

				foodrating.setFooditemrating(Double.parseDouble(rate));
				String foodReview = review.next();
				foodrating.setFooditemreview(foodReview);
				...
				if (foodItem.getFooditemrating() == null) {
					...
					if (foodItem.getNumofrating() == null) {
					...
					foodRating = ((foodItem.getFooditemrating() * foodItem.getNumofrating()) + Double.parseDouble(rate))
							/ (foodItem.getNumofrating() + 1);
